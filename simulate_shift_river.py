...
def detect_made_hand(hole_cards, board_cards):
    ...
    if 4 in counts: return ["quads"]
    if 3 in counts and 2 in counts: return ["full_house"]
    if max(suit_counts.values())>=5: return ["flush"]
    if is_straight(values): return ["straight"]
    if 3 in counts: return ["set"]
    if counts.count(2)>=2: return ["two_pair"]
...
def is_overcard_river(hole_cards, river_card):
    if hole_cards[0].rank!=hole_cards[1].rank: return False
...
def is_straight(values):
    ...
    for i in range(len(unique_values)-4):
        if unique_values[i]-unique_values[i+4]==4: return True
    if set([14,2,3,4,5]).issubset(set(values)): return True
    return False
